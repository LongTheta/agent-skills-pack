...
Creates project rules in `.cursor/rules/` to provide persistent context for the AI agent. Produces `.mdc` rule files with frontmatter and actionable content. Tier 1: proposals only; user reviews before applying; no overwrite without confirmation.
...
3. **Create rule file** — `.mdc` in `.cursor/rules/` with YAML frontmatter and body.
...
- **Limitations:** Rules are Cursor-specific; format differs for other IDEs. Max ~50 lines recommended; split large rules into focused pieces. Cannot execute code; rules are static guidance only.
